...
const defaultPinoConfig = {
  ...
  redact: ['access_token', 'password', 'token', 'secret'],
  changeLevelName: 'serverity'
  // useOnlyCustomLevels: true
};

function onRequestFinish(req, res, next) {
...
function attachValidationErrorDetails(err) {
  if (err.details) {
    return ({ details: err.details })
  }
}
...
module.exports = ({
  ...
  log.reqResLogger = () => function reqResLogger(res, req, next) {
  ...
  log.errorLogger = () => function errorLogger(err, req, res, next) {
    const errorLog = log.child({ type: 'errorLogger' })
    errorLog.error({
      message: err.stack,
      ...attachValidationErrorDetails(err),
      context: {
        httpRequest: {
          method: req.method,
          url: req.url,
